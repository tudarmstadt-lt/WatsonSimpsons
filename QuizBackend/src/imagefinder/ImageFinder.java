...
public final class ImageFinder {
	...
	private final static String backendUrl = "http://simpsonswiki.com/w/api.php?action=query&format=json&";
 ...
 */
	private static ArrayList<Imagetitle> getEpisodeImage(String episode) {
		String query = backendUrl.concat(imagesOnArticleQuery).concat(episode);
		ArrayList<Imagetitle> images = new ArrayList<Imagetitle>();
		ImagesOnPage page = ImageBackendCommunicator
				.getImagesOnArticleResponse(query);
		if (page.getImages() != null) {
			...
			images.remove(new Imagetitle(
					"File:The Saga of Carl - title screen.png"));
			...
			images.remove(new Imagetitle(
					"File:800px-Flag of the United Kingdom.svg.png"));
			...
			images.remove(new Imagetitle(
					"File:The Simpsons Archive favicon.png"));
 ...
 */
	private static PageScaledImage getScaledImages(int scaledWidth,
			String episode) throws UnsupportedEncodingException {
		String resultString = backendUrl.concat(thumbImageQuery)
				.concat(scaledWidth + "&titles=")
				.concat(URLEncoder.encode(episode, "UTF8"));
		return ImageBackendCommunicator.getThumbImageResponse(resultString);
	}
/**
...
/**
 * find a Simpson picture according to a String
 * if String is null return fix Image
 * special treatment for episode names 
 * (gets images of the Simpsonswiki articles)
 * @param answer
 ...
 */
	...
	private static Image findPicture(String name) throws IOException {
		...
		Image response = ImageBackendCommunicator
				.getAllImagesResponse(
						backendUrl.concat(allImagesQuery).concat(queryResult))
				.getQuery().getAllimages().get(0);
		String answer = response.getTitle().substring(5).replace(".png", "")
				.replace(".gif", "").replace(".jpg", "");
		...
		response = ImageBackendCommunicator
				.getAllImagesResponse(
						backendUrl.concat(allImagesQuery).concat(queryResult))
				.getQuery().getAllimages().get(0);
		return response;
	 ...
	 * 1) firstname -> File:firstname lastname f.i.: Homer -> File:Homer Simpson 
	 * 2) lastname ->  File:firstname lastname f.i.: Flanders -> File:Ned Flanders 
	 * 3) name -> File:name.p (no jpg/3d pictures) f.i.: Crazy Cat Lady -> File:Crazy_Cat_Lady.p
	 * 
	 * @param answer
	 * String of an NE
